...
完整报告除满足各用例标准外，还必须使用 [`report-quality-rubric.md`](report-quality-rubric.md) 按书型评分：总分至少 28/35、任一项不得低于 3 分，且“全书统摄性”与“支撑材料完整度”必须达到 4 分。

## 用例 0：宽泛读书请求默认完整报告

提示：

> 用 `$ai-reading` 帮我读一下《小红书爆款规律》。
...
> 用 `$ai-reading` 读《爆款抖音短视频》，让我能把书里的方法真正用于一个招聘类抖音账号。
...
> 用 `$ai-reading` 自动测试方法论、文学、科学和哲学四种书型的全部四阶段，不需要等待我回答。请使用测试夹具并给出通过、失败和状态继承结果。
...
- 被跳过的阶段标记为 `skipped` 或 `auto_foundation`，不得写成 `completed`。
